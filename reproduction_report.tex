...
The paper evaluates on three GraphRAG frameworks: Microsoft GraphRAG, GFM-RAG,
and HippoRAG~2.  We attempted to install and run each:

\begin{itemize}[nosep]
  \item \textbf{Microsoft GraphRAG} (v3.0.9): Successfully installed in
    Python~3.12 venv.  Full indexing requires extensive LLM calls
    (entity extraction, community summarization) and was not completed
    due to API cost constraints.
  \item \textbf{GFM-RAG}: Requires Python~3.12 and CUDA toolkit.
    Installation blocked by \texttt{langchain/SQLAlchemy} dependency conflicts.
  \item \textbf{HippoRAG~2}: Requires Python~3.10.  Installation blocked
    by \texttt{torch} version conflicts with existing environment.
\end{itemize}
