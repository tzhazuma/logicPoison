...
\subsection{Naive RAG Evaluation Attempt}

To compare with the paper's Naive RAG ASR numbers (82--99\%, Table~1), we set up a
Naive RAG pipeline using \texttt{facebook/contriever} for dense retrieval and
FAISS for vector search---matching the paper's retrieval configuration.

\textbf{Completed:}
\begin{itemize}[nosep]
  \item Contriever model loaded and verified (768-dimensional embeddings)
  \item FAISS index built for all 3 poisoned corpora (6,119--11,656 vectors each)
  \item Document retrieval pipeline tested: top-$k$=10 retrieval working correctly
  \item Evaluation script created at \texttt{eval/run\_naive\_rag.py}
\end{itemize}

\textbf{Blocked:} LLM answer generation could not be completed due to API credit
limitations on the opencode-go account.  The retrieval infrastructure is ready;
resuming the evaluation requires API credits to generate answers for 50--500
questions per dataset.

\textbf{For reference}, the paper's Naive RAG ASR targets (using GPT-4o-mini) are:
HotpotQA~92.0\%, 2Wiki~90.0\%, MuSiQue~99.2\%.  With the retrieval pipeline
verified and the poisoned corpus prepared, running the full evaluation is
a straightforward extension once API access is restored.
