...
\begin{itemize}[nosep]
  \item \textbf{2Wiki: ASR 90.0\%} --- matches the paper's 90.0\% exactly,
    suggesting comparable attack effectiveness for this dataset.
  \item \textbf{HotpotQA: ASR 60.0\%} --- significantly \emph{lower} than the
    paper's 92.0\%.  The \texttt{deepseek-v4-flash} model appears more robust
    against poisoned context for HotpotQA-style questions, correctly answering
    4/10 despite entity swaps.
  \item \textbf{MuSiQue: evaluation incomplete} --- requires token-aware retry.
  \item \textbf{Model difference matters:} Using a different LLM
    (\texttt{deepseek-v4-flash} vs.\ \texttt{GPT-4o-mini}) significantly impacts ASR.
    The reasoning model's tendency to ``think through'' the context may help it
    bypass poisoned entities.
  \item \textbf{Token budget is critical:} Reasoning models consume output tokens
    for internal thinking.  Without sufficient \texttt{max\_tokens} (we used up to
    8,000), answers appear empty, inflating ASR artificially.
\end{itemize}
